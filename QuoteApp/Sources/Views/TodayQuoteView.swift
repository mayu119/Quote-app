...
struct TodayQuoteView: View {
    ...
    private func quoteLayer(quote: Quote) -> some View {
        Group {
            if userSettings.isVerticalTextMode {
                verticalQuoteLayer(quote: quote)
            } else {
                horizontalQuoteLayer(quote: quote)
            }
        }
        .opacity(appear ? 1 : 0)
        .offset(y: appear ? 0 : 20)
    ...
    private func horizontalQuoteLayer(quote: Quote) -> some View {
    ...
    private func verticalQuoteLayer(quote: Quote) -> some View {
        let foldedText = JapaneseLineBreaker.fold(quote.quoteJa, maxPerLine: 11)
        ...
        .environment(\.layoutDirection, .rightToLeft) // 横並びを右→左の順にする
    }
...
struct VerticalTextView: View {
                ...
                VStack(spacing: 0) {
                    ...
                    ForEach(Array(chars.enumerated()), id: \.offset) { _, char in
                        Text(String(char))
                            ...
                            .rotationEffect(needsRotation(char) ? .degrees(90) : .zero)
                            .offset(charOffset(char))
                            .frame(height: fontSize * 1.1) // 文字同士の縦の間隔を調整
                    }
                }
            }
        }
    ...
    /// 回転が必要な文字（長音記号、括弧など）
    private func needsRotation(_ char: Character) -> Bool {
        let rotatables: Set<Character> = ["ー", "-", "~", "〜", "(", ")", "「", "」", "『", "』", "（", "）", "＜", "＞", "[", "]"]
        return rotatables.contains(char)
    }

    /// 句読点や小書き文字（ぁぃぅ等）の位置微調整
    private func charOffset(_ char: Character) -> CGSize {
        let punctuations: Set<Character> = ["。", "、", ".", ","]
        if punctuations.contains(char) {
            // 右上へシフト
            return CGSize(width: fontSize * 0.6, height: -fontSize * 0.6)
        }
        let smallKanas: Set<Character> = ["ぁ", "ぃ", "ぅ", "ぇ", "ぉ", "っ", "ゃ", "ゅ", "ょ", "ァ", "ィ", "ゥ", "ェ", "ォ", "ッ", "ャ", "ュ", "ョ"]
        if smallKanas.contains(char) {
            // 右上へシフト
